...
class CCDataset(Dataset):
    def __init__(self):
        data_df = pd.read_csv('/opt/spark/data/creditcard.csv')
...
if __name__ == "__main__":
    ...
    spark = SparkSession \
        .builder \
        .appName("Ray on spark example (ray train)") \
        .config("spark.task.cpus", "4") \
        .getOrCreate()

    num_workers = 1

    setup_ray_cluster(num_worker_nodes=num_workers, head_node_options={"include_dashboard": True, "dashboard_port": 8265})
    ...
    # first bring down Ray cluster, then the Spark cluster
